...
| proposal background 1차 후보 | ❌ 고정 하드웨어 41장 개선 0%p로 승격 거부, 운영 모델 유지 |
| clean hardware-adapt v2 | 🔄 AI Hub 89,779장 + 단일 하드웨어 hard sample, material-only 20 epoch 학습 중 |
| clean v2 승격 범위 | ⛔ 통과해도 offline 후보만 생성, 독립 end-to-end 검증 전 자동 배포 금지 |
...
scripts/
  ...
  nas/watch_proposal_verifier_hardware_adapt.sh clean AI Hub replay+단일 하드웨어 적응+오프라인 게이트
