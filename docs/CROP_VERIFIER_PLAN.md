...
## 7. 종이(paper) 정책 공간 소진 조사 (2026-09-05)

paper가 품목 중 가장 약해 재학습 없이 런타임 정책만으로 개선 여지가 있는지
실제 원본 이미지로 라이브 파이프라인을 직접 조사했다. 오프라인 crop 시뮬레이션은
저장된 manifest crop과 라이브 YOLO bbox가 어긋날 수 있어 신뢰하지 않고, 하드웨어
검증셋(학습 미사용) 9건 전부 원본 이미지를 `pipeline.run()`에 통과시켜 확인했다.

- 현재 배포 파이프라인의 실제 paper 정확도: **4/9(44%)**. 오프라인 crop 기준
  "검증기 단독 56%"는 라이브 bbox와 달라 실제로 재현되지 않았다.
- 남은 오답 4건 중 1건만 검증기가 실제로 맞혔으나 신뢰도(0.447)가 구제 임계값
  미달이었다. 나머지 3건은 **검증기 자체가 틀렸다**(plastic으로 오판, 신뢰도
  0.46~0.95) — 임계값을 아무리 조정해도 고칠 수 없다.
- RESCUE_CONF을 0.45~0.65로 스윕해도 paper 정확도는 4/9로 고정이었다(§ 아래).
  paper의 병목은 정책이 아니라 **모델이 실제 키오스크 종이 재질 자체를 인식하지
  못하는 것**이다.
- 8/3 종이 3배 오버샘플 실패(AI Hub 검증 97.9% ↔ 실제 키오스크 1/9)와 결이 같다.
  AI Hub 종이 데이터를 더 넣는 방향은 이미 반증됐고, 오늘 백본 교체·증강도
  효과가 없었다(§4.1). **현재 모델·데이터로 paper를 더 개선할 방법이 없다.**
  실제 키오스크 카메라로 찍은 종이(영수증·크라프트지·구겨진 종이) 표본 없이는
  이 병목을 풀 수 없다.

### RESCUE_CONF 재보정 (라이브 파이프라인 전수 실측)

같은 조사 과정에서 하드웨어 검증셋 35건 전부를 원본 이미지로 재실측했다.

| RESCUE_CONF | 정답 | 오답 | 보류 | 위험허용 |
|---|---:|---:|---:|---:|
| 0.45 | 25 | 3 | 7 | 1 |
| 0.50 | 25 | 3 | 7 | 1 |
| 0.55 | 25 | 3 | 7 | 1 |
...
0.50이 plastic 한 건을 추가로 살리며 다른 품목·위험도에 영향이 없어 기본값을
0.60→0.50으로 낮췄다(McNemar p=1.000, 유의성은 없으나 손해도 없음).

### 안전 관련 발견 — styrofoam→plastic 고신뢰 오탐

같은 조사에서 검증셋의 styrofoam 1건이 신뢰도 0.85로 plastic 판정을 받아
`ALLOWED`로 통과하는 것을 확인했다(RESCUE_CONF 값과 무관하게 0.60/0.50 양쪽에서
동일하게 존재하는 기존 결함). YOLO·검증기 둘 다 신뢰도가 TRUST_CONF 이상이라
현재 어떤 가드도 이 경로를 막지 못한다. styrofoam 표본이 검증셋에 3건뿐이라
당장 정책으로 대응하기엔 근거가 부족하지만, **수거거부 품목이 고신뢰로 허용
품목으로 새는 사례**라 우선순위 높은 후속 조사 대상으로 남긴다.

정정(2026-09-06): 이 건 포함 styrofoam 검증셋 3건 중 2건이 사실 라벨 오류였다.
실물을 다시 보니 구겨진 완충재 시트라 재질을 특정하기 어려웠고, 1건은 비닐에
가까웠다. §9 정정 참고 — 모델 결함이 아니라 정답 라벨 문제였다.

## 8. TrashNet 추가로 종이 실개선 (2026-09-05)

§7에서 paper는 "AI Hub 데이터·백본·정책 세 방향 다 막다른 길"이라고 결론지었으나,
**새로운 데이터 소스**(같은 라이선스 문제나 도메인 반복 없이)를 찾아 실제로 풀었다.

### 데이터 소스

- **TrashNet** (garythung/trashnet, MIT 라이선스, 인용 요구): 흰 배경·실내조명·
  아이폰 촬영 단일 객체 2,527장. AI Hub 야외/컨베이어 사진과 정반대 조건이라
  겹치지 않는 신호다. Hugging Face에서 3.63GB zip 다운로드, 무결성 검증
  (zipfile.testzip 통과), 직접 열어 확인(신문지·피자소스 캔 등 실제 확인).
- **AI Hub 컨베이어 추출본**(`TS_1.영상추출_02.종이`, 16,940장)은 확인 결과
  한 프레임에 여러 물체가 뒤섞여 있어 단일 객체 crop 파이프라인에 안 맞고
  분할 작업이 추가로 필요해 이번엔 보류했다.
- 클래스 매핑: paper+cardboard→paper(997장), plastic→plastic(482), glass→glass(501),
  metal→can(410). dent/label/foreign_material 정답이 없어 -1(마스킹).
  vinyl/styrofoam/battery/fluorescent에 대응 클래스가 없어 제외.

### 학습

hard100과 동일 조건(증강 없음, MobileNetV3-Small, 같은 초기 체크포인트·
하이퍼파라미터) + TrashNet manifest를 `--oversample-spec ...=2`로 추가.
`--selection-material-target paper`로 체크포인트 선택에 paper 재현율을 반영했다.
20 epoch 완료, exit 0.

### 실측 — 하드웨어 검증셋 35건, 실제 원본 이미지로 라이브 파이프라인 그대로

| | 배포본(hard100) | TrashNet 추가 |
|---|---:|---:|
| 정답 | 25 | **28** |
| 오답 | 3 | **2** |
| 보류 | 7 | **5** |
| 위험허용 | 1 | 1 (동일 — 새로 생긴 위험 없음) |

McNemar: 배포본만 맞은 건 **0건**, TrashNet만 맞은 건 3건, p=0.25. 유의성 기준은
못 넘지만 후보가 단 한 건도 배포본에 진 적이 없다는 게 핵심이다.

품목별(paper, n=9): 4정답/1오답/4보류 → **7정답/1오답/1보류**. 대부분 저신뢰
보류였던 건이 정답으로 확정되며 개선됐다.

crop 단독 비교(참고용, 실제 배포 신호 아님)에서는 material 74.3%→82.9%,
paper 56%→89%, plastic 71%→86%였다. styrofoam은 3건 중 2건이 기존부터 있던
위험(§7에서 이미 기록한 결함과 동일 건)이고 1건만 새로 저신뢰→고신뢰 오답으로
바뀌었으나, **실제 라이브 파이프라인에서는 위험 건수가 늘지 않았다** — crop
분석과 라이브 결과가 다시 한번 어긋난 사례로, 최종 판단은 항상 라이브 파이프라인
기준으로 한다.

### 결정: 배포

`weights/verifier_qwen35_mnv3_v1.onnx`를 이 체크포인트로 교체했다. 테스트 117개
통과. styrofoam 고신뢰 오탐(§7)은 이번에도 해결되지 않은 별도 과제로 남는다.

### 다음 방향

같은 방법(라이선스가 명확한, AI Hub와 다른 촬영조건의 외부 데이터셋 실측 검증
후 채택)을 다른 취약 품목에도 적용할 수 있다. styrofoam 전용 데이터셋을 우선
찾아볼 가치가 있다.

## 9. styrofoam 외부 데이터 보강 — 실패, 원인은 물량이 아니라 도메인 (2026-09-05~06)

§8의 방법(외부 데이터셋 실측 검증 후 채택)을 styrofoam에 그대로 적용했으나
**두 차례 모두 배포본을 못 이겼다.** paper 성공 사례와 달리 이번엔 원인이
분명해져 문서로 남긴다.

### 데이터 소스

- Roboflow Universe styrofoam 데이터셋(337장, CC BY 4.0, `source.roboflow.com`
  직접 서빙 — Cloudflare 보호 없음, `universe.roboflow.com` 앱 페이지만 보호돼
  있어 계정 생성·CAPTCHA 우회 없이 페이지 열람만으로 수집). 서로 다른 미러
  URL로 두 차례에 걸쳐 랜덤 다른 부분집합을 수집.
  - 1차: 53장 수집 → 육안 검수로 8장 제외(워터마크·비닐봉지 지배·라벨 불일치)
    → **45장**.
  - 2차: 50장 수집(1차와 4장만 중복) → 20장 제외(광고 배너·워터마크 8장,
    검정/짙은색 재질 불명확 5장, 도면·목업 3장, 무관 피사체 3장, 비닐봉지
    지배 1장) → **30장**.
  - 합산 **75장**(train 64 / val 11).
- TrashNeXt는 검토 후 배제했다. 라이선스 문제 이전에 "Foam_Rubber" 클래스가
  고무신발·타이어를 styrofoam과 같이 라벨링해 데이터 자체가 오염돼 있었다
  (직접 확인).
- AI Hub의 기존 styrofoam 사진(`TS_2.직접촬영_05.스티로폼`)도 재확인했으나
  이미 base manifest에 전량 포함돼 있고, paper와 동일한 실외/아스팔트 배경
  패턴이라 새 신호가 아니었다.

### 배경 조사 — "장수가 너무 적지 않냐"는 지적에 대한 실측

사용자가 75장이 적어 보인다고 지적해 전체 manifest의 실제 클래스별 물량을
재확인했다:

| manifest | styrofoam | 비고 |
|---|---:|---|
| base manifest (합성/AI Hub) | **12,000** | plastic/pet/glass/can과 완전 동수 (paper는 오히려 11,980로 더 적음) |
| **hardware manifest (실제 Pi5 캡처)** | **4** | 전체 클래스 중 최하위 (다른 클래스 20~27) |
| 이번 Roboflow 외부 보강 | 75 |  |

즉 원본 물량 자체는 전혀 부족하지 않다(다른 주요 클래스와 동수). 진짜 병목은
**base manifest의 대다수가 AI Hub 실외 촬영이라 키오스크 카메라 도메인과
안 맞고, 우리 하드웨어에서 실제로 캡처된 진짜 styrofoam은 4장뿐**이라는 것.
아래 실측이 이 가설을 두 번 확인해준다.

### 학습

§8과 동일 레시피(증강 없음, MobileNetV3-Small, TrashNet 체크포인트 warm-start,
동일 하이퍼파라미터) + styrofoam manifest를 `--oversample-spec`으로 추가,
`--selection-material-target paper styrofoam`. 45장판(80x)과 75장판(50x,
장수 증가분에 비례 축소) 모두 20 epoch 완주, exit 0.

### 실측 — 하드웨어 검증셋 35건, 라이브 파이프라인

| | 배포본(TrashNet) | styro45단독 | styro75통합 |
|---|---:|---:|---:|
| 정답 | **28** | 28 | 27 |
| 오답 | 2 | 4 | 4 |
| 보류 | 5 | 3 | 4 |
| 위험허용 | **1**(`6d5372228df0`) | 2(`d00c965eda99` 신규) | 2(동일) |

styrofoam 세부(n=3): 배포본 1정답/1오답/1보류 → 45장·75장 **둘 다 동일하게**
1정답/2오답/0보류. 30장을 더 넣어도 이 3건의 판정은 한 건도 안 바뀌었다.
McNemar 세 쌍 모두 유의하지 않으나(p=1.0 x2, 0.999...) 방향이 뚜렷하다 —
75장판은 45장판에도 졌고(plastic 정답 1건 추가 손실), 45장판은 배포본에
못 미쳤다.

핵심 패턴: 두 후보 모두 배포본에서 "보류"였던 애매한 케이스를 **확신에 찬
오답**으로 바꿨다(`d00c965eda99`가 대표 사례 — 위험 등급까지 상승). 외부
데이터가 판별력을 높인 게 아니라 과신만 키웠다는 뜻으로, 도메인이 안 맞는
데이터를 아무리 정제해서 늘려도 실제 하드웨어 판정에는 반영되지 않는다는
가설과 정확히 일치한다.

### 결정: 미배포

두 후보 모두 배포본을 이기지 못해 `weights/verifier_qwen35_mnv3_v1.onnx`는
§8의 TrashNet 체크포인트 그대로 유지한다. 코드/가중치 변경 없음.

### 다음 방향

라이선스 검증된 외부 인터넷 사진으로 styrofoam을 개선하는 경로는 이번
조사로 소진됐다고 판단한다(45장, 75장 모두 동일하게 무효). 남은 가장 유력한
경로는 **실제 키오스크 카메라로 styrofoam 실물을 직접 캡처해 hardware
manifest를 4장에서 늘리는 것**뿐이다 — 도메인 불일치가 근본 원인이라는
진단이 두 번의 실측으로 확인됐기 때문에, 도메인이 일치하는 데이터가 아니면
소용이 없다.

### 정정 (2026-09-06)

위 실측에 쓴 하드웨어 검증셋 styrofoam 3건 중 2건이 라벨 오류였다. 실물을
다시 확인하니 구겨진 완충재 시트라 재질 특정이 어려웠고(정상적으로는
"일반" 처리 대상), 1건은 비닐이었다. 두 건 다 manifest에서 제외하고 비닐
1건은 재라벨링했다. 재평가하니 세 후보 모두 위험 건수가 0으로 나왔다 —
§7부터 계속 "미해결 안전 이슈"로 기록해온 styrofoam→plastic 고신뢰 오탐이
사실 이 라벨 오류 때문이었다. 45장/75장 미배포 결정 자체는 안 바뀌지만
(정답 개수 기준으로 여전히 배포본을 못 이김), 검증셋에 실제 styrofoam
validation 샘플이 지금 0개라 styrofoam 정확도는 당장 측정 불가능하다.

## 10. 탐지기 confidence 재검토 (2026-09-07)

§5 v3 기록에 conf를 0.25에서 0.10으로 낮추면 진단 정확도가 60.98%에서 68.29%로
오른다는 측정이 있어 현재 NCNN 배포본으로 다시 재봤다. 라벨 정정 후 39장 기준
탐지기 단독 정확도는 conf 0.25에서 58.97%, 0.15에서 61.54%로 확실히 올랐고
negative specificity는 33.3%로 변화 없었다. 놓친 박스도 4건에서 1건으로 줄었다.

그런데 라이브 파이프라인 33건으로 확인하니 정답 수가 27로 양쪽 동일했고, 오히려
paper 보류 1건이 오답으로 바뀌었다. 검증기가 이미 탐지기 약점을 메꾸고 있어서
탐지 단계 개선분이 최종 결과로 넘어오지 않는다. DETECT_CONF는 0.25로 유지한다.

투명 플라스틱 3장은 conf를 낮추면 박스는 생기지만(2건) 전부 vinyl로 잡히고,
파이프라인에서는 결국 양쪽 다 보류로 끝난다. 투명 재질은 탐지·검증 양쪽 모두
현재 데이터로는 한계다.
